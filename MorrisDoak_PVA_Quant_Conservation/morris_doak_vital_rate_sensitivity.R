...
## first plot effects of directly changing vital rates -- Figure 9.1 
n<-length(goose.vr); vr<-seq(0,1,.1)
...
for (h in 1:n) {
  ...
  for (i in 1:length(vr)) {
    goose.vr2[[h]]<-vr[i] 
    A<-matrix(sapply(goose.el, 
                     eval,
                     goose.vr2 , 
                     NULL), nrow=sqrt(length(goose.el)), byrow=TRUE) 
    vrsen[i,h] <- max(Re(eigen(A)$values))
  } }


matplot(rownames(vrsen), vrsen, type="l", lwd=2, las=1, 
        ylab="Goose population growth", 
        xlab="Value of vital rate", 
        main="Effects of directly chaning goose vital rates") 
...
x
sum(x$elasticity)
barplot(t(x[,2:3]), beside=TRUE, las=1, xlab="Vital rate", main="Kestral vital rate sensitivity and elasticity") 
